...
export default function BranchHubView() {
  const {
    branches,
    currentBranchId,
    users,
  ...
  const branchList = useMemo(() => {
    ...
    return branches.filter((branch) => branch.members?.includes(currentUser?.id));
  }, [branches, currentUser?.id, canManageBranches]);
  ...
  const branchAnnouncements = useMemo(() => {
    ...
    return announcements
      .filter((announcement) => announcement.branchId === selectedBranch.id)
  ...
  const branchMessages = useMemo(() => {
    ...
    return messages
      .filter((message) => message.type === 'branch' && message.branchId === selectedBranch.id)
  ...
  const branchTasks = useMemo(() => {
    ...
    return tasks.filter((task) => !task.isSubTask && task.branchId === selectedBranch.id);
  ...
  const handleAnnouncementSubmit = (event) => {
    ...
    if (!selectedBranch || !announcementContent.trim()) {
      return;
    }
    postBranchAnnouncement({
  ...
  const handleSendMessage = (event) => {
    event.preventDefault();
    if (!selectedBranch || !chatMessage.trim()) {
      return;
    }
    addBranchMessage({
      branchId: selectedBranch.id,
    ...
    return (
      <div className="space-y-6 fade-in">
        <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-2xl font-bold text-white">Branch Hub</h2>
          ...
          <div className="flex flex-col items-stretch gap-2 md:flex-row md:items-center md:justify-end">
            <button
              type="button"
              className="branch-hub__exit-button"
            ...
            {canManageBranches ? (
              ...
              >
                <i className="fas fa-plus mr-2" />Add Branch
              </button>
            ) : null}
          </div>
        </header>
        ...
        {branchList.length === 0 ? (
          <div className="perplexity-card p-6 text-center text-gray-400">
            ...
            <p>{canManageBranches ? 'Create your first branch to get started.' : 'You are not part of any branches yet.'}</p>
          </div>
        ) : (
            ...
            {branchList.map((branch) => (
              ...
              >
                ...
                <div className="mt-4 flex -space-x-2">
                  {branch.members?.slice(0, 4).map((memberId) => {
                  ...
                  {branch.members && branch.members.length > 4 ? (
                ...
                <p className="mt-4 text-sm text-gray-400">
                  Tap to view announcements, chat, and branch tasks.
                </p>
              </button>
            ))}
          </div>
  ...
  const isBranchCreator = selectedBranch.createdBy === currentUser?.id;
    ...
    <div className="space-y-6 fade-in">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          >
            <i className="fas fa-arrow-left mr-2" />Back to branch list
          </button>
        ...
        <div className="flex flex-col items-stretch gap-2 md:flex-row md:items-center md:justify-end">
          ...
          <div className="rounded-xl border border-gray-700/60 bg-gray-900/40 px-4 py-2 text-sm text-gray-300">
            <span className="font-semibold text-white">Branch ID:</span>{' '}
            <span className="font-mono text-purple-300">{selectedBranch.id}</span>
          </div>
        </div>
      </header>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-4">
          ...
          <div className="perplexity-card p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-bold text-white">Branch Chat</h3>
        ...
        <div className="space-y-4">
          <div className="perplexity-card p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-bold text-white">Branch Tasks</h3>
              <span className="text-xs uppercase tracking-wide text-gray-400">{branchTasks.length} assigned</span>
            </div>
            {branchTasks.length === 0 ? (
              <div className="rounded-lg border border-dashed border-gray-700/70 bg-gray-900/40 p-6 text-center text-sm text-gray-400">
                No tasks have been assigned to this branch yet.
              </div>
            ) : (
